...
alpha = np.real(eigval)[0]
beta = np.imag(eigval)[0]
a = jnp.array(np.real(eigvec)[:,0]).reshape((1,-1))
b = jnp.array(np.imag(eigvec)[:,0]).reshape((1,-1))
...
def exact_solution_temp(state_init, time_indexes):
	...
	"""
	exp_term = jnp.exp(alpha * time_indexes)
	cos_betat = jnp.cos(beta * time_indexes)
	sin_betat = jnp.sin(beta * time_indexes)
	eig_vect1 = (a * cos_betat - b * sin_betat) * exp_term
	eig_vect2 = (a * sin_betat + b * cos_betat) * exp_term

	# eig_vect1 = jnp.vstack( (jnp.sin(3 * time_indexes)*jnp.exp(2 * time_indexes), jnp.cos(3 * time_indexes)*jnp.exp(2 * time_indexes)) ).transpose()
	# eig_vect2 = jnp.vstack( (-jnp.cos(3 * time_indexes)*jnp.exp(2 * time_indexes), jnp.sin(3 * time_indexes)*jnp.exp(2 * time_indexes)) ).transpose()
	coeff_val = jax.numpy.linalg.solve(jnp.array([[eig_vect1[0,0], eig_vect2[0,0]], [eig_vect1[0,1], eig_vect2[0,1]]]), state_init)
	return coeff_val[0] * eig_vect1 + coeff_val[1] * eig_vect2
...
def main_fn(path_config_file, extra_args={}):
	...
	xTest, xNextTest = exact_solution(m_init_test_x, mdata_log.time_step, mdata_log.trajectory_length, mdata_log.n_rollout)
	...
	if num_data_colocation > 0:
		...
		xcoloc, _ = exact_solution(m_init_coloc_x, mdata_log.time_step, mdata_log.trajectory_length, mdata_log.n_rollout)
	...
	# xTrain_num, _ = numeric_solution(m_init_train_x, mdata_log.time_step, mdata_log.trajectory_length, mdata_log.n_rollout)
	# traj_set_num = np.array(xTrain_num[indx_trajectory*mdata_log.trajectory_length:(indx_trajectory+1)*mdata_log.trajectory_length])
	# Do some plotting
	...
	for i in range(nstate):
		...
		# plt.plot(time_index, traj_set_num[:,i], color='blue', linewidth=2)
		plt.xlabel('Time (s)')
	...
	# plt.plot(traj_set_num[:,0], traj_set_num[:,1], color='blue', linewidth=2)
